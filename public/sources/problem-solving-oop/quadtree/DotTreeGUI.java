...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		...
		if (mode == 'a') {
			...
			else if (tree!=null) tree.insert(new Dot((double) x,(double) y));
	...
	private void test0() {
		...
		System.out.println("After adding A, size = " + tree.size());
		tree.insert(new Dot(150,450)); // B
		System.out.println("After adding B, size = " + tree.size());
		tree.insert(new Dot(250,550)); // C
		System.out.println("After adding C, size = " + tree.size());
		int bad = 0;
	...
	private void test1() {
		...
		System.out.println("After adding D, size = " + tree.size());
		tree.insert(new Dot(200,250)); // E
		System.out.println("After adding E, size = " + tree.size());
		tree.insert(new Dot(350,175)); // F
		System.out.println("After adding F, size = " + tree.size());
		tree.insert(new Dot(500,125)); // G
		System.out.println("After adding G, size = " + tree.size());
		tree.insert(new Dot(475,250)); // H
		System.out.println("After adding H, size = " + tree.size());
		tree.insert(new Dot(525,225)); // I
		System.out.println("After adding I, size = " + tree.size());
		tree.insert(new Dot(490,215)); // J
		System.out.println("After adding J, size = " + tree.size());
		tree.insert(new Dot(700,550)); // K
		System.out.println("After adding K, size = " + tree.size());
		tree.insert(new Dot(310,410)); // L
		System.out.println("After adding L, size = " + tree.size());
	...
	/**
	 * test tree 2 -- tests more additional points and an edge case when the x,y is on the line.
	 * 
	 */
	private void test2() {
		System.out.println("Beginning test 2");
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(500,300), 0,0,800,600);    // start with A
		System.out.println("After adding A, size = " + tree.size());
		tree.insert(new Dot(250,150));                                   // B  top left quadrant of A
		System.out.println("After adding B, size = " + tree.size());
		tree.insert(new Dot(350,250));                                   // C  bottom right quadrant of B
		System.out.println("After adding C, size = " + tree.size());
		
		// doesn't get inserted because it's an indeterminate case: on the edge, (we can decide which  quadrant it should go)
		tree.insert(new Dot(300, 300)); 
		
		
		System.out.println("The number of nodes in the PointQuadTree = " + tree.size()); // tree size is still 3
		System.out.println("The list of nodes in the PointQuadTree = " + tree.allPoints()); // all points
		int bad = 0;
		
		bad += testFind(500, 200, 20, 3, 3, 0);		  // rect for all; circle for all; find none
		bad += testFind(500, 300, 50, 3, 3, 1);		  // rect for all; circle for A,B; find A
		bad += testFind(600, 350, 100, 2, 1, 0 );     // rect for 2; circle for A; find None
		
		// edge case: Exactly one the line of a quadrant
		bad += testFind( 300, 300, 60, 3, 3, 0);     // rect for all; circle for all; find None
	...
	public void handleKeyPress(char key) {
		...
		else if (key=='0') { // tries test 0 when 0  is pressed
			test0();
		}
		else if (key=='1') { // tries test 1 when 1  is pressed
			test1();
		}
		else if (key=='2') { // tries test 2 when 2 is pressed
			test2();
		}
		// TODO: YOUR CODE HERE -- your test cases
		else if ( key == 'd') // refresh the tree - destroy it
			tree = null;
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		g.fillOval((int) (tree.getPoint().getX() - dotRadius/ 2.0), (int)(tree.getPoint().getY() - dotRadius/2.0), dotRadius, dotRadius);
		
		g.drawLine((int)tree.getPoint().getX(), tree.getY2(), (int)tree.getPoint().getX(), tree.getY1());
