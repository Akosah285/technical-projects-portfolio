 ...
 * @authors Akwasi Akosah and Sylvester Elorm Coch
 */
public class CollisionGUI extends DrawingGUI {
	...
	public void draw(Graphics g) {
		...
		if(colliders != null) {
			for (Blob blobcolliders : colliders) {
				// Ask the colliders to draw themselves in red.
				g.setColor(new Color(255,0,0));
				blobcolliders.draw(g);
		}
		}
	}
	...
	private void findColliders() {
		// TODO: YOUR CODE HERE
		// Create the tree that hold blobs
		colliders = new ArrayList<Blob>();
		PointQuadtree<Blob> blobsTree = new PointQuadtree<Blob>(new Blob(getX(),getY()),0,0,width,height);
		
		// insert blobs to the tree
		for (Blob blob : blobs) {
			blobsTree.insert(blob);			
		}
		// For each blob, see if anybody else collided with it
		for (Blob blob : blobs) {
			List<Blob> collidersInRegion = blobsTree.findInCircle(blob.getX(),blob.getY(),2* blob.getR());
			
			// go through the list of colliders and exclude the blob of focus 
			for (Blob blobsCollided : collidersInRegion) {
				if(blob.getX()!= blobsCollided.getX() && blob.getY()!= blobsCollided.getY()) {					
					colliders.add(blobsCollided);
				}
			}
		}
	}
