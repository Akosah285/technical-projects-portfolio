...
uint8_t lsm303_AccelRegisterRead(uint8_t RegisterAddress) {		
	...
	i2cSend(RegisterAddress);
	i2cStart();
	i2cSend((LSM303_ADDRESS_ACCEL << 1) | 00000001);
	data = i2cReadNoAck();
	i2cStop();
	return (uint8_t)(data);
}
...
int lsm303_AccelInit() {
	// Check the WHOAMI register	
  	if (lsm303_AccelRegisterRead(LSM303_WHO_AM_I_A) != 0x33)
  	{
		...
		// 0x57 => Normal mode, 100 Hz sampling, all three axes active
		// 0x24 =>  Normal mode, 10 Hz sampling, Z axis only
		uint8_t ctrl_reg1 = 0x57;
	
		//  Enable low-power mode (must also disable HR mode)
		//  ctrl_reg1 |= 0x08;		    // Set the LPen bit
	
		lsm303_AccelRegisterWrite(LSM303_CTRL_REG1_A, ctrl_reg1);
  
		// Control register 4 (Datasheet 8.9)
		// Set the full scale sensor range (±2g)
		uint8_t ctrl_reg4 = 0x00;
		
		// Disable high-resolution mode, leave everything else as-is
		

		// Enable high-resolution mode, leave everything else as-is
		ctrl_reg4 |= (1 << 3);
